...
public:
    static AppConfig LoadFromAppData(std::wstring appID)
    {
        ...
        std::wstring t = json.getW("title");
        if (!t.empty())
            c.title = t;
        std::wstring d = json.getW("discordClientId");
        if (!d.empty())
            c.discordClientId = d;
        std::wstring e = json.getW("entryPoint");
        if (!e.empty())
            c.entryPoint = e;
        ...
        c.useLocalFileSystem = json.getBool("useLocalFileSystem");
